...
public class Lab_6 extends GLJPanel implements GLEventListener  {
    ...
    public void wallCreate(GL2 gl2, float x1,float y1, float x2, float y2, float x3 ,float y3,float h)
    {
        gl2.glBegin(GL2.GL_TRIANGLES);
        gl2.glVertex3f(x1,  0,y1);
        gl2.glVertex3f(x2, h,y2);
        gl2.glVertex3f(x3,  0,y3);
        gl2.glEnd();
    }


    public void draw(int x, int prom, GL2 gl2)
    {
        float[] xPoints = new float[x];
        float[] yPoints = new float[x];
        int height = 4;
        for (int i = 1; i <= x; i++)
        {
            xPoints[i - 1] = (float) (prom * Math.sin((2 * Math.PI / x) * i));
            yPoints[i - 1] = (float) (prom * Math.cos((2 * Math.PI / x) * i));
        }
        gl2.glRotatef(360/x,0,1,0);
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE,materials[2], 0);
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR,materials[2], 0);
        wallCreate(gl2, xPoints[0], yPoints[0],0,0, xPoints[1], yPoints[1],height);
        ...
        wallCreate(gl2, xPoints[1], yPoints[1],0,0, xPoints[2], yPoints[2],height);

        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE,materials[9], 0);
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR,materials[9], 0);
        wallCreate(gl2, xPoints[2], yPoints[2],0,0, xPoints[3], yPoints[3],height);

        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE,materials[11], 0);
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR,materials[11], 0);
        wallCreate(gl2, xPoints[3], yPoints[3],0,0, xPoints[4], yPoints[4],height);

        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE,materials[1], 0);
        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR,materials[1], 0);
        wallCreate(gl2, xPoints[4], yPoints[4],0,0, xPoints[0], yPoints[0],height);
    }
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        float[] lightLevel =  {2f,2f,2f,1f};
        float[] lightLevel2 =  {3f,3f,3f,1f};
        gl2.glLightfv (GL2.GL_LIGHT1, GL2.GL_AMBIENT, lightLevel,0);
